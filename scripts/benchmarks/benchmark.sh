...
# Benchmark AgentOS against the local mock provider.
# Usage:
...
GATEWAY_LOG="$TMP_DIR/agentos.log"
...
(
  cd "$ROOT_DIR"
  go run ./scripts/tooling/mock_provider.go -listen 127.0.0.1:18080 -latency 25ms >"$MOCK_LOG" 2>&1
) &
MOCK_PID=$!

(
  cd "$ROOT_DIR"
  go run ./cmd/agentos --config configs/benchmark.yaml >"$GATEWAY_LOG" 2>&1
) &
GATEWAY_PID=$!
