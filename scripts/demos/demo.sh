#!/bin/bash
# AgentOS Demo - Agent Execution Governance
# This script demonstrates how AgentOS controls agent actions

set -e

ADMIN_URL="${AgentOS_ADMIN_URL:-http://localhost:8081}"
API_KEY="${AgentOS_API_KEY:-demo-key-001}"
GREEN='\033[0;32m'
RED='\033[0;31m'
YELLOW='\033[1;33m'
BLUE='\033[0;34m'
...
echo ""
echo -e "${BOLD}========================================${NC}"
echo -e "${BOLD}  AgentOS Demo - Agent Governance${NC}"
echo -e "${BOLD}========================================${NC}"
echo ""
...
curl -s -X POST "$ADMIN_URL/admin/v1/test-action" \
  ...
  -d '{"protocol":"git","tool":"github.list_repos","target":"AgentOS/AgentOS","capability":"read"}' | jq .
pause
...
curl -s -X POST "$ADMIN_URL/admin/v1/test-action" \
  -H "Content-Type: application/json" "${AUTH[@]}" \
  -d '{"protocol":"git","tool":"github.delete_repo","target":"AgentOS/AgentOS","capability":"delete"}' | jq .
pause
...
RESULT=$(curl -s -X POST "$ADMIN_URL/admin/v1/test-action" \
  -H "Content-Type: application/json" "${AUTH[@]}" \
  -d '{"protocol":"git","tool":"github.create_pull_request","target":"AgentOS/AgentOS","capability":"write"}')
echo "$RESULT" | jq .
