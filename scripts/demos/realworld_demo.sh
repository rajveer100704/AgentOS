#!/usr/bin/env bash
# Real-world MCP testing demo for AgentOS
# Sends real MCP tool calls through AgentOS to a mock upstream MCP server.
#
# Usage:
...
AgentOS_URL="${AgentOS_URL:-http://localhost:8080}"
...
echo -e "${BOLD}AgentOS Real-World MCP Testing Demo${NC}"
echo "This demo sends MCP tool calls through AgentOS to a mock GitHub MCP server."
echo "It exercises the full governance pipeline: allow, review, and block decisions."
divider

# ── Health checks ──
echo -e "${BOLD}[1/7] Health checks${NC}"
echo -n "  AgentOS gateway... "
if curl -sf "${AgentOS_URL}/health" > /dev/null 2>&1; then
  echo -e "${GREEN}OK${NC}"
else
  echo -e "${RED}FAILED${NC} (is AgentOS running?)"
...
echo ""
RESPONSE=$(curl -sf -X POST "${AgentOS_URL}/v1/mcp" \
...
RESPONSE=$(curl -sf -X POST "${AgentOS_URL}/v1/mcp" \
  ...
  -d '{
    ...
    "params": {
      ...
      "arguments": {"repo": "saivedant169/AgentOS"}
    }
  }' 2>&1) || RESPONSE=$(echo "$RESPONSE")
echo "  Response: $(echo "$RESPONSE" | jq -r '.result.content[0].text // .error.message // .' 2>/dev/null || echo "$RESPONSE")"
divider
...
RESPONSE=$(curl -sf -X POST "${AgentOS_URL}/v1/mcp" \
  ...
  -d '{
    ...
    "params": {
      ...
      "arguments": {"repo": "saivedant169/AgentOS", "title": "Agent-created PR", "body": "Automated by coding agent"}
    }
  }' 2>&1) || RESPONSE=$(echo "$RESPONSE")
...
echo ""
RESPONSE=$(curl -s -X POST "${AgentOS_URL}/v1/mcp" \
  -H "Content-Type: application/json" \
  -H "X-API-Key: ${API_KEY}" \
  -d '{
    "jsonrpc": "2.0",
...
RESPONSE=$(curl -sf -X POST "${AgentOS_URL}/v1/mcp" \
  ...
  -d '{
    ...
    "params": {
      ...
      "arguments": {"repo": "saivedant169/AgentOS", "branch": "feature/agent-work"}
    }
  }' 2>&1) || RESPONSE=$(echo "$RESPONSE")
echo "  Response: $(echo "$RESPONSE" | jq -r '.result.content[0].text // .error.message // .' 2>/dev/null || echo "$RESPONSE")"
divider
