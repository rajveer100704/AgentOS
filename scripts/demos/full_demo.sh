...
cleanup() {
    ...
    wait "$AGENTOS_PID" 2>/dev/null || true
    echo "Done."
}
trap cleanup EXIT
...
if kill -0 "$AGENTOS_PID" 2>/dev/null; then
    success "AgentOS running (PID: $AGENTOS_PID)"
    echo "  Gateway:  http://localhost:8080"
    echo "  Admin:    http://localhost:8081"
else
    echo -e "${RED}Failed to start. Check /tmp/AgentOS_demo.log${NC}"
    exit 1
fi
...
echo "curl http://localhost:8080/v1/models -H 'X-API-Key: agentos-test-default-001'"
echo ""
curl -s http://localhost:8080/v1/models \
  -H "X-API-Key: agentos-test-default-001" | python3 -m json.tool
echo ""
...
"
echo ""
success "Real AI response through AgentOS → Ollama"
...
echo ""
echo "Server log:"
grep "pii" /tmp/AgentOS_demo.log | tail -1 || echo "  (PII warning logged)"
echo ""
...
for i in $(seq 1 62); do
    CODE=$(curl -s -o /dev/null -w "%{http_code}" -X POST http://localhost:8080/v1/chat/completions \
      -H "Content-Type: application/json" \
      -H "X-API-Key: agentos-test-default-002" \
...
echo "curl http://localhost:8081/metrics | grep AgentOS"
echo ""
curl -s http://localhost:8081/metrics | grep "^AgentOS_" | head -10
echo ""
...
echo -e "${GREEN}"
echo "  AgentOS — Open-Source AI Gateway"
echo ""
