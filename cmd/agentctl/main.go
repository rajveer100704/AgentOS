...
func main() {
	...
	adminURL := getEnv("AGENTOS_ADMIN_URL", defaultAdminURL)
	gatewayURL := getEnv("AGENTOS_GATEWAY_URL", defaultGatewayURL)

	switch os.Args[1] {
	case "plugin":
		if len(os.Args) < 3 {
			fmt.Println("Usage: agentctl plugin <search|info|install|list|outdated|remove> [args]")
			os.Exit(1)
		}
		var err error
		switch os.Args[2] {
	...
	case "approve":
		if len(os.Args) < 3 {
			fmt.Println("Usage: agentctl approve <id> [comment]")
			os.Exit(1)
		}
		comment := ""
		if len(os.Args) > 3 {
	...
	case "deny":
		if len(os.Args) < 3 {
			fmt.Println("Usage: agentctl deny <id> [comment]")
			os.Exit(1)
		}
		comment := ""
		if len(os.Args) > 3 {
	...
	case "evidence":
		if len(os.Args) < 3 {
			fmt.Println("Usage: agentctl evidence <sessions|export|report> [args]")
			os.Exit(1)
		}
		switch os.Args[2] {
		case "sessions":
		...
		case "export":
			if len(os.Args) < 4 {
				fmt.Println("Usage: agentctl evidence export <session-id> [--file output.json]")
				os.Exit(1)
			}
			cmdEvidenceExport(adminURL, os.Args[3], os.Args[4:])
		case "report":
			if len(os.Args) < 4 {
				fmt.Println("Usage: agentctl evidence report <session-id> [--html] [--file output.md]")
				os.Exit(1)
			}
	...
	case "policy-pack":
		if len(os.Args) < 3 {
			fmt.Println("Usage: agentctl policy-pack <list|show> [args]")
			os.Exit(1)
		}
		switch os.Args[2] {
		case "list":
		...
		case "show":
			if len(os.Args) < 4 {
				fmt.Println("Usage: agentctl policy-pack show <name> [--dir DIR]")
				os.Exit(1)
			}
	...
	case "policy":
		if len(os.Args) < 3 {
			fmt.Println("Usage: agentctl policy <history|current|rollback> [args]")
			os.Exit(1)
		}
		switch os.Args[2] {
		case "history":
		...
		case "rollback":
			if len(os.Args) < 4 {
				fmt.Println("Usage: agentctl policy rollback <version>")
				os.Exit(1)
			}
	...
	case "manifest":
		if len(os.Args) < 3 {
			fmt.Println("Usage: agentctl manifest <create|list|drift> [args]")
			os.Exit(1)
		}
		switch os.Args[2] {
		case "create":
		...
		case "drift":
			if len(os.Args) < 4 {
				fmt.Println("Usage: agentctl manifest drift <manifest-id>")
				os.Exit(1)
			}
	...
	case "supply-chain":
		if len(os.Args) < 3 {
			fmt.Println("Usage: agentctl supply-chain <list|sign|verify> [args]")
			os.Exit(1)
		}
		switch os.Args[2] {
		case "list":
...
func printUsage() {
	fmt.Println(`agentctl — AgentOS CLI
...
Commands:
  ...
  approve     Approve a pending item: agentctl approve <id> [comment]
  deny        Deny a pending item: agentctl deny <id> [comment]
  policy      Policy versioning (history, current, rollback)
  simulate    Simulate a policy decision with full trace
  why         Show decision trace for a past action: agentctl why <envelope-id>
  diff-policy Diff two policy files: agentctl diff-policy <old.yaml> <new.yaml>
...
Environment:
  AGENTOS_GATEWAY_URL  Gateway URL (default: http://localhost:8080)
  AGENTOS_ADMIN_URL    Admin URL (default: http://localhost:8081)`)
}
...
func cmdStatus(gatewayURL, adminURL string, jsonOut bool) {
	...
	if pendingCount > 0 {
		fmt.Printf("  %d pending (run: agentctl pending)\n", pendingCount)
	} else {
