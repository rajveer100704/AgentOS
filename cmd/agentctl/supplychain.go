...
func cmdSupplyChainSign(args []string) {
	if len(args) < 1 {
		fmt.Println("Usage: agentctl supply-chain sign <file> --key <hex-key> [--name NAME] [--version VERSION] [--type TYPE]")
		os.Exit(1)
	}
...
func cmdSupplyChainVerify(args []string) {
	if len(args) < 1 {
		fmt.Println("Usage: agentctl supply-chain verify <file> --signature <sig-file> --key <hex-key>")
		os.Exit(1)
	}
