...
func pluginSearch(args []string) error {
	if len(args) < 1 {
		return fmt.Errorf("usage: agentctl plugin search <query>")
	}
...
func pluginInfo(args []string) error {
	if len(args) < 1 {
		return fmt.Errorf("usage: agentctl plugin info <name>")
	}
	reg, err := fetchRegistry("")
	if err != nil {
		return err
...
func pluginInstall(args []string) error {
	if len(args) < 1 {
		return fmt.Errorf("usage: agentctl plugin install <name> [--plugins-dir DIR] [--plugins-config FILE]")
	}
	name := args[0]
	pluginsDir := "plugins"
...
func pluginRemove(args []string) error {
	if len(args) < 1 {
		return fmt.Errorf("usage: agentctl plugin remove <name>")
	}
	name := args[0]
	pluginsConfig := "plugins.yaml"
