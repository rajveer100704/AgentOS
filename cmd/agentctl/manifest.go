...
func cmdManifestCreate(adminURL string, args []string) {
	...
	if taskID == "" {
		fmt.Println("Usage: agentctl manifest create --task <TICKET-ID> [--tools \"glob,...\"] [--protocols \"proto,...\"] [--verbs \"verb,...\"] [--max-actions N] [--owner NAME] [--description DESC] [--risk-tier low|medium|high] [--expires-in 1h]")
		os.Exit(1)
	}
