...
func cmdTestAction(adminURL string, args []string) {
	...
	if protocol == "" || tool == "" || target == "" {
		fmt.Println("Usage: agentctl test-action [--dry-run] --protocol <mcp|shell|sql|git|http> --tool <tool_name> --target <target> [--capability <read|write|delete|deploy>] [--params key=value,...]")
		os.Exit(1)
	}
