...
func cmdSimulate(adminURL string, args []string) {
	...
	if protocol == "" || tool == "" || target == "" {
		fmt.Println("Usage: agentctl simulate --protocol <protocol> --tool <tool> --target <target> [--capability <cap>]")
		os.Exit(1)
	}
...
func cmdWhy(adminURL string, args []string) {
	if len(args) < 1 {
		fmt.Println("Usage: agentctl why <envelope-id>")
		os.Exit(1)
	}
	id := args[0]
...
func cmdDiffPolicy(args []string) {
	if len(args) < 2 {
		fmt.Println("Usage: agentctl diff-policy <old.yaml> <new.yaml>")
		os.Exit(1)
	}
