...
- Install AgentOS
- Connect Claude Code
- Watch AgentOS block a destructive action
...
- Build `bin/AgentOS` and `bin/agentctl`
- Start AgentOS and the mock MCP server in the background
...
Done. Now open Claude Code in this directory and type:
  'list my github repos using AgentOS tools'
```
...
The mock returns a list of repos. AgentOS logs an `allow` decision because
`github.list_*` is allow-listed in `pr-writer.yaml`.

### b. REVIEW -- creating a PR

> create a pull request titled "demo: add hello world" against main using AgentOS tools

AgentOS returns a `review` decision and parks the call as a pending approval.
...
> delete the staging repo using AgentOS tools

AgentOS blocks immediately. The decision is logged. The agent gets a clean
...
AgentOS decided, why, and (for reviews) who approved it.
