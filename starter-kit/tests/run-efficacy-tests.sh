...
API_KEY="${AgentOS_API_KEY:-starter-key-001}"
...
if ! curl -sf "$ADMIN_URL/health" >/dev/null 2>&1; then
    echo -e "${RED}Error: AgentOS admin API is not responding at $ADMIN_URL${NC}"
    echo "Start AgentOS first: cd starter-kit && ./install.sh"
    exit 1
fi

echo ""
echo -e "${BOLD}AgentOS Efficacy Test Suite${NC}"
echo "=============================="
echo ""
...
# Generate report
cat > "$REPORT_FILE" <<EOF
# AgentOS Efficacy Report

**Date:** $(date -u +"%Y-%m-%dT%H:%M:%SZ")
**Policy:** starter-kit (demo config)
**AgentOS URL:** $ADMIN_URL
...
$(if [ "$FAIL" -eq 0 ]; then echo "All $TOTAL tests passed. AgentOS correctly enforced every policy decision."; else echo "**$FAIL test(s) failed.** Review the failed tests above and adjust your policy configuration."; fi)
EOF
