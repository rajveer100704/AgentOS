...
For setup help that is not covered here, open an [install problem](https://github.com/saivedant169/AgentOS/issues/new?template=install_problem.yml). For unexpected runtime behavior, open a [bug report](https://github.com/saivedant169/AgentOS/issues/new?template=bug_report.yml).
...
- Another AgentOS process is already running (`pgrep AgentOS`).
...
```bash
make build
./bin/AgentOS --config configs/demo.yaml
```
...
2. **Read the startup warnings.** On startup AgentOS walks `tool_policies.rules` and logs `[config] tool policy rule #N references unknown protocol ...` for anything outside `mcp | http | shell | sql | git | *`. If you see this line, that rule never matches — fix the protocol field.
3. **Confirm the policy is the one you think.** `agentctl policies` lists what the running process has loaded.
4. **Use `agentctl simulate` to dry-run an envelope.** Cheaper than restarting the gateway every time you tweak a rule.
...
|-------------------|-------|-----|
| `session not found` | Wrong `--session` ID, or the session is on a different host. | `agentctl evidence sessions` to list available sessions; copy the exact ID. |
...
If `chain_valid: false` shows up under steady-state operation with no operator changes, that is a security event. Treat it as one: snapshot the database, do not let the process keep writing, and file an [install problem](https://github.com/saivedant169/AgentOS/issues/new?template=install_problem.yml) with the verify output.
