# Running AgentOS behind nginx or Caddy

AgentOS exposes **three separate ports** out of the box:
...
```nginx
# /etc/nginx/sites-available/AgentOS.conf

# Upstreams for the three AgentOS ports.
upstream AgentOS_gateway { server 127.0.0.1:8080; keepalive 32; }
upstream AgentOS_admin   { server 127.0.0.1:8081; keepalive 16; }
upstream AgentOS_metrics { server 127.0.0.1:8082; keepalive 8;  }
...
server {
    ...
    location /metrics {
        proxy_pass http://AgentOS_metrics/metrics;
    }
}
```
...
After the proxy is up, point your editor at `https://your-host/sse` (the proxied MCP endpoint) and confirm an action round-trip from agent to AgentOS goes through with no perceptible lag. If the agent times out, the SSE buffering is almost certainly the cause.
...
- **TLS terminated at proxy but app expects HTTPS** → set `X-Forwarded-Proto https` so AgentOS generates correct callback URLs (GitHub App, dashboards).
...
- Broken / outdated snippet → open a [bug report](https://github.com/saivedant169/AgentOS/issues/new?template=bug_report.yml).
- Different reverse proxy (Traefik, Envoy, HAProxy) you would like covered → open a [feature request](https://github.com/saivedant169/AgentOS/issues/new?template=feature_request.yml).
