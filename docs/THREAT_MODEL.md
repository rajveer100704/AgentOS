...
- End-user authentication to upstream services (AgentOS governs agent access, not human access)

---

## Trust Boundaries

### Agent <-> AgentOS (UNTRUSTED)
...
### AgentOS <-> Upstream Tools (SEMI-TRUSTED)

AgentOS proxies requests to upstream services (GitHub API, databases, shell, HTTP endpoints) using task-scoped, short-lived credentials. AgentOS controls what the agent can request, but upstream services enforce their own authorization. AgentOS does not trust upstream services to enforce agent-level policy.

### Operator <-> AgentOS Control Plane (TRUSTED)

Operators interact with AgentOS through the admin API (port 8081), `agentctl` CLI, and GraphQL. Access is authenticated via API keys and enforced through RBAC (admin, operator, viewer roles). All admin actions are audited.

### Policy Packs / WASM Plugins <-> AgentOS (CONDITIONALLY TRUSTED)

Extensions run within AgentOS's process. WASM plugins execute in a sandboxed runtime (wazero). Policy packs are loaded at startup. In strict mode, unsigned extensions are rejected.
...
**Residual risk:** Exfiltration through side channels not covered by network policy (e.g., DNS tunneling). Mitigated by monitoring and network-level controls outside AgentOS.
...
**Residual risk:** Manifest declarations that are too broad. Mitigated by policy simulation tools (`agentctl simulate`) and operator review of manifest scope.
...
- `agentctl evidence verify` CLI detects any modification to the chain
...
**Description:** A malicious policy pack or WASM plugin is loaded into AgentOS, subverting the governance layer.
...
**Description:** An attacker gains access to the AgentOS admin API or control plane, allowing them to modify policies, approve actions, or disable governance.
...
| Threat | Likelihood | Impact | AgentOS Controls | Residual Risk |
|--------|-----------|--------|--------------------|----|
...
- Quarterly, as part of the AgentOS security review cycle
