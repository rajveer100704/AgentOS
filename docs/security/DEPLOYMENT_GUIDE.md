...
This guide covers the recommended architecture, configuration, and operational procedures for deploying AgentOS in production environments. AgentOS is a single Go binary with YAML configuration, but production deployments require attention to network segmentation, TLS, secrets management, monitoring, and incident response.
...
|--------|----------|-------|
| Single binary | Small teams, dev/staging | `make build && ./AgentOS -config config.yaml` |
...
AgentOS exposes three distinct network surfaces. Each should be isolated.
...
Use your existing load balancer or ingress controller (AWS ALB, Nginx, Envoy, Istio) to terminate TLS. AgentOS communicates over plaintext within the trusted network.
...
### Option 2: TLS termination at AgentOS
...
server:
  ...
  tls:
    enabled: true
    cert_file: "/etc/AgentOS/tls/server.crt"
    key_file: "/etc/AgentOS/tls/server.key"
    min_version: "1.2"
```
...
3. AgentOS's credential broker issues task-scoped, short-lived credentials to agents
...
| AgentOS admin API keys | Vault / K8s Secret | 90 days |
...
AgentOS exposes metrics at `/metrics` on the admin port:
...
|--------|----------------|----------|
| `AgentOS_policy_decisions_total{decision="block"}` | Spike > 3x baseline | Warning |
| `AgentOS_approval_queue_depth` | > 50 pending | Warning |
| `AgentOS_evidence_chain_errors` | > 0 | Critical |
| `AgentOS_credential_issuance_failures` | > 0 | Critical |
| `AgentOS_request_latency_p99` | > 100ms | Warning |
| `AgentOS_session_drift_events` | Any | Warning |
| `AgentOS_kill_switch_activations` | Any | Critical |
...
AgentOS outputs structured JSON logs (Zap). Ship to your log aggregation platform (Elasticsearch, Loki, Splunk, Datadog):

```bash
# Example: ship logs via fluent-bit sidecar
AgentOS -config config.yaml 2>&1 | fluent-bit -i stdin -o elasticsearch ...
```
...
pg_dump -Fc AgentOS_evidence > /backups/evidence_$(date +%Y%m%d).dump
...
```bash
agentctl evidence verify --all-sessions
```
...
AgentOS's gateway and admin API are stateless. Run multiple replicas behind a load balancer:
...
- Monitor alerts from Prometheus, SIEM, and AgentOS's anomaly detection
- Check `/admin/v1/alerts` for recent alerts
- Determine severity level

**2. Containment**
- **Kill switch:** Terminate suspicious sessions immediately via `agentctl kill-session <id>`
...
**3. Investigation**
- Export evidence for affected sessions: `agentctl evidence export --session <id>`
- Verify evidence chain integrity: `agentctl evidence verify --session <id>`
...
- Test updated policies with `agentctl simulate`
...
- On-call rotation for AgentOS operators
