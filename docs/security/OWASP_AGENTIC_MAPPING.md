...
3. **`agentctl approve` / `agentctl deny`** CLI commands for operator workflow
...
3. **`agentctl evidence verify`** CLI validates the entire chain and reports any breaks
...
7. **`agentctl` plugin marketplace** with visibility into trust status
