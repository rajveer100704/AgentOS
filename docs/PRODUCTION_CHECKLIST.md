...
Use this checklist before exposing AgentOS outside a local demo.

## Configuration

- Set `AgentOS_CONFIG` to the mounted production config path.
...
config:
  tenant:
    apiKeySecret:
      enabled: true
      name: AgentOS-tenant-key
      key: api-key
      envName: AgentOS_DEFAULT_API_KEY
```

Create the secret:

```bash
kubectl create secret generic AgentOS-tenant-key \
...
- Review the [operations runbook](OPERATIONS_RUNBOOK.md) for backup, restore, and incident response commands.
