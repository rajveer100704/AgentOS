...
AgentOS can send signed policy events to an HTTP endpoint. This is optional and can be tested locally without any hosted service.
...
Start AgentOS with the local webhook config:
...
The receiver prints the event payload plus `X-AgentOS-Signature` and `X-AgentOS-Timestamp` headers.
...
- Keep webhook receivers non-blocking; AgentOS sends events asynchronously.
