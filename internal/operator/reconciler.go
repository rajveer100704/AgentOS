...
func (r *Reconciler) Reconcile(ctx context.Context) error {
	...
	if err != nil {
		// Create
		cm.Data = map[string]string{"agentos.yaml": string(yamlBytes)}
		if createErr := r.client.Create(ctx, cm); createErr != nil {
			return fmt.Errorf("creating configmap: %w", createErr)
		}
		log.Printf("operator: created configmap agentos-config")
	} else {
		// Update
		existing.Data = map[string]string{"agentos.yaml": string(yamlBytes)}
		if updateErr := r.client.Update(ctx, existing); updateErr != nil {
			return fmt.Errorf("updating configmap: %w", updateErr)
		}
		log.Printf("operator: updated configmap agentos-config")
	}
