...
func TestReconcileCreatesConfigMap(t *testing.T) {
	...
	err := cl.Get(context.Background(), keyFor("agentos-config", "default"), &cm)
	if err != nil {
		t.Fatalf("configmap not created: %v", err)
	}
	yamlData, ok := cm.Data["agentos.yaml"]
	if !ok || len(yamlData) == 0 {
		t.Fatal("configmap missing agentos.yaml data")
	}
}
...
func TestReconcileUpdatesExistingConfigMap(t *testing.T) {
	...
	existing := &corev1.ConfigMap{
		ObjectMeta: metav1.ObjectMeta{Name: "agentos-config", Namespace: "default"},
		Data:       map[string]string{"agentos.yaml": "old"},
	}
