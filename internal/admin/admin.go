...
// WithTraceStore sets the telemetry TraceStore on the admin server.
func WithTraceStore(store *telemetry.TraceStore) ServerOption {
	return func(s *Server) {
		s.traceStore = store
	}
}
...
func (s *Server) Router() http.Handler {
	...
	if s.traceStore != nil {
		th := NewTraceHandler(s.traceStore)
		r.Get("/admin/v1/traces", th.ListTraces)
		r.Get("/admin/v1/traces/stats", th.TraceStats)
		r.Get("/admin/v1/edge/metrics", th.EdgeMetrics)
	}
