...
// the X-AgentOS-Signature and X-AgentOS-Timestamp headers are set
...
func TestHMACSignaturePresent(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		mu.Lock()
		sigHeader = r.Header.Get("X-AgentOS-Signature")
		tsHeader = r.Header.Get("X-AgentOS-Timestamp")
	...
	if sigHeader == "" {
		t.Fatal("X-AgentOS-Signature header is missing")
	}
	if tsHeader == "" {
		t.Fatal("X-AgentOS-Timestamp header is missing")
	}

	// Parse timestamp and verify the signature
	ts, err := strconv.ParseInt(tsHeader, 10, 64)
	if err != nil {
		t.Fatalf("X-AgentOS-Timestamp is not a valid integer: %v", err)
	}
...
func TestNoSignatureWithoutSecret(t *testing.T) {
	...
	if sigHeader != "" {
		t.Errorf("X-AgentOS-Signature should be empty without secret, got %q", sigHeader)
	}
	if tsHeader != "" {
		t.Errorf("X-AgentOS-Timestamp should be empty without secret, got %q", tsHeader)
	}
}
...
func TestNoSignatureWithEmptySecret(t *testing.T) {
	...
	if sigHeader != "" {
		t.Errorf("X-AgentOS-Signature should be empty with empty secret, got %q", sigHeader)
	}
}
