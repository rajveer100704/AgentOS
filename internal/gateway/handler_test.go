...
func TestChatCompletionCacheHit(t *testing.T) {
	...
	if w1.Header().Get("X-AgentOS-Cache") != "MISS" {
		t.Errorf("first request: expected cache MISS header, got %q", w1.Header().Get("X-AgentOS-Cache"))
	}
	...
	if w2.Header().Get("X-AgentOS-Cache") != "HIT" {
		t.Errorf("second request: expected cache HIT header, got %q", w2.Header().Get("X-AgentOS-Cache"))
	}
}
...
func TestChatCompletionBudgetWarnings(t *testing.T) {
	...
	warning := w.Header().Get("X-AgentOS-Budget-Warning")
